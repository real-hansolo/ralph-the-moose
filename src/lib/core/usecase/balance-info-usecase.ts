...
import { TBalanceInfoRequest } from "../usecase-models/balance-info-usecase-models";
...
export default class BalanceInfoUsecase implements BalanceInfoInputPort {
  ...
  async execute(request: TBalanceInfoRequest): Promise<void> {
    ...
    const inscriptionBalance = inscriptionBalanceDTO.data.balance;
    ...
    const claimableAmount = claimableAmountDTO.data.amount;
